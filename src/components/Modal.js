...
class ModalCard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
    this.re = new RegExp("http://swapi.dev/api/([a-z]*)/");
    this.type = "";
    this.list = [];
    this.name = "";
    this.data = {};
  ...
  getData = () => {
    ...
    const temp = this.testSet.filter(
      (thing) => thing["url"] === this.props.url
    );
    this.data = temp[0];
  };
  ...
  render() {
    ...
    if (this.data.url !== undefined) {
              ...
              corresValue = corresValue.map((v) => {
                return (
                  <LinkModal
                    ...
                    url={v}
                    data={this.testSet}
                    clickedmodal={this.otherModal}
                  />
                );
              });
            }
          } else if (String(corresValue).includes("http")) {
            corresValue = (
              <LinkModal
                url={corresValue}
                clickedmodal={this.otherModal}
      ...
      this.name =
        this.type === "films" ? this.data["title"] : this.data["name"];
      this.color = this.props.master.filter(
        (i) => i.type === this.type
      )[0].tachColor;
    }
    return (
      ...
      >
        <Modal.Header closeButton className={`h3 bg-${this.color}`}>
          <Modal.Title>
            <span className={`pa1 ${this.color} bg-black di br2`}>
              #{this.type}
            </span>{" "}
            {this.name}
          </Modal.Title>
